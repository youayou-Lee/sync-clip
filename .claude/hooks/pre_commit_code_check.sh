...
# Purpose: Check code quality before git commits using Flake8, black, and isort
...
if [[ -z "$staged_files" ]]; then
    print_status "$GREEN" "✅ No Python files staged for commit"
    exit 0
fi

print_status "$BLUE" "📝 Checking Python files:"
...
if [[ "$large_files_found" == true ]]; then
    print_status "$RED" "❌ Some files are too large. Consider splitting large files or adding them to .gitignore"
    exit 1
fi
...
if [[ "$flake8_failed" == true ]]; then
    ...
    print_status "$YELLOW" "💡 Tip: You can run 'flake8 --max-line-length=$BLACK_LINE_LENGTH --ignore=E203,W503 .' to check all files"
    exit 1
else
...
for file in $staged_files; do
    if [[ -f "$file" ]]; then
        if grep -n -i "TODO\|FIXME\|XXX" "$file" >/dev/null 2>&1; then
            print_status "$YELLOW" "⚠️  Found TODO/FIXME comments in $file:"
...
for file in $staged_files; do
    if [[ -f "$file" ]]; then
        if grep -n "print(" "$file" >/dev/null 2>&1; then
            print_status "$YELLOW" "⚠️  Found print() statements in $file (consider removing for production):"
            grep -n "print(" "$file" | head -5 | sed 's/^/    /'
            issues_found=true
        fi
    fi
done
...
if [[ "$issues_found" == true ]]; then
    print_status "$YELLOW" "⚠️  Some potential issues found. Review above warnings."
else
...
# Final summary
print_status "$GREEN" ""
print_status "$GREEN" "🎉 All code quality checks passed!"
print_status "$GREEN" "✅ Your code is ready to commit"
